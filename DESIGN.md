...
The implementation of part two of the project also uses UDP sockets to setup a multicast group. The program can be launched by passing in the multicast group address, the port number, the total number of processes in the group, and (optionally) a flag to make the program use total ordering and a flag to designate itself as the sequencer. If the last two parameters, i.e. the total ordering flag and the sequencer flag are passed then the processes will wait to print the message after ordering else they will print as soon as they receive it. All the processes will start a send and receive thread. The send thread will wait for 3 seconds, then send a limited number of messages to the multicast group. The receive thread will have a loop that keeps waiting to receive until no messages have been received for 5 seconds. Once 5 seconds have passed, the program will close the socket, the log file, and it will exit.

The ordered version of the program uses a `std::vector` to store the incoming messages. The messages are added to the vector when they are received. The message that the sender sends is its address, process id, logical clock count, and a arbitrary message, e.g. "0.0.0.0:2456:23:MESSAGE_5". When running the ordered version, the script also runs a process with an extra argument that designates it as the sequencer. The sequencer process sends all the messages that it receives as a multicast. When the client processes receive the message from both the sequencer and the sender, it will check if the global sequencer number is one grater than its own sequencer number. If yes, it will deliver the message and update its sequence number.
...
The main issue I encountered was somehow getting unordered outputs when not using the total ordering. Even though there were not ordering algorithms in place, the messages were always being delivered in order when running all the processes on one machine.
